...
def retrieve_best_word(prefix: str) -> tuple[str, float]:
    ...
    next_word, info_gain = _compute_and_best_word_for_prefix(prefix=prefix)
...
def _compute_and_best_word_for_prefix(prefix: str) -> tuple[str, float]:
...
def _progress_tracker(n_words: int, result_queue: Queue, result: list):
    pbar = tqdm(total=n_words, desc='Word list processing', position=2, leave=False)
